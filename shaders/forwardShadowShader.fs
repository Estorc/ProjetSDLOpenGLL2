...
layout (location = 1) out vec4 FragLightings;
...
{    
    ...
    vec3 ndc = fs_in.FragClipSpacePosition.xyz / fs_in.FragClipSpacePosition.w;

    // The NDC coordinates (ndc.xy) are in the range [-1, 1]
    // You can now use these NDC coordinates directly, or convert them to screen space if needed
    
    // In case you need to use NDC as screen space (without needing the screen size):
    vec2 screenSpace = (ndc.xy + 1.0) * 0.5; // Now in range [0, 1]


    vec4 lightMean = imageLoad(lightMap, ivec2(gl_FragCoord.xy));
    imageStore(intermediateLightMap, ivec2(gl_FragCoord.xy), vec4(Lo, 1.0)); // Store the light color in the light map
    
    vec3 ambient = vec3(0.03) * albedo * ao;
    vec3 color = ambient + lightMean.rgb;
