...
#define CLASS_IMPORT_HEADER_FILENAME "import_classes.h"
#define CLASS_TYPE_HEADER_FILENAME "classes_types.h"
		...
		if (need_rewriting_imports) {
			...
			for (size_t i = 0; i < import_struct.unique_method_count; i++){
				...
				for (size_t j = 0; j < import_struct.class_count; j++) {
					...
					fprintf(import_classes_header_file, "%s", import_struct.corresponding_method[i][method_index]);
			...
			for (size_t i = 0; i < import_struct.class_count; i++) {
				fprintf(import_classes_header_file, "%d", find_string_index(import_struct.extends[i], import_struct.class_name, import_struct.class_count));
